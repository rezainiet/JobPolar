...
const MyJobs = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`https://job-polar-server.vercel.app/myJobs/${user?.email}`)
      .then((res) => res.json())
      .then((data) => {
        // console.log(data);
        setJobs(data);
...
const handleDelete = (id) => {
  // console.log(id)
  fetch(`https://job-polar-server.vercel.app/job/${id}`, {
    method: "DELETE",
  })
    .then((res) => res.json())
    .then((data) => {
    ...
    <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4">
      <div className="my-jobs-container">
        ...
        <div className="flex justify-center text-black space-x-8">
          {currentPage > 1 && (
            <button onClick={prevPage} className="hover:underline cursor-pointer">
              Previous
            </button>
          )}
          {indexOfLastItem < jobs.length && (
            <button onClick={nextPage} className="hover:underline cursor-pointer">
              Next
            </button>
          )}
        </div>
